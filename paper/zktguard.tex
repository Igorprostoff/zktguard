...
manuscript so it can iterate independently.

\subsection{Circuit structure}

The \texttt{account\_age} circuit witnesses the ChaCha20-Poly1305
session tuple $(\mathit{key}, \mathit{nonce}, \mathit{ct},
\mathit{aad}, \mathit{tag})$ as private inputs and enforces, in
order: (i) \texttt{ChaCha20Poly1305Decrypt} recovers the plaintext
and asserts the Poly1305 tag matches, so a forged tag or tampered
ciphertext cannot yield a witness; (ii) \texttt{JsonTimestampExtract}
locates the \texttt{"created\_at":} marker at a witnessed offset in
the authenticated plaintext and decodes the ten ASCII digits that
follow; (iii) the age threshold is applied to that decoded value.
ChaCha20 is bit-sliced --- each 32-bit word is carried as 32 boolean
signals so rotations are free rewiring and \textsc{xor} costs one
constraint per bit --- and Poly1305 uses a five-limb 26-bit
accumulator with lazy reduction modulo $2^{130}-5$. On BLS12-381 the
full circuit measures $\approx\!2.49\times10^{5}$ constraints, well
inside the $10^{6}$ target; the primitives are validated against the
RFC~8439 test vectors.

\subsection{Trust boundary and the cipher-suite constraint}

The circuit only verifies ChaCha20-Poly1305, and the reference
attestor only ever emits ChaCha20-Poly1305 sealings, so the
``ChaCha20-Poly1305 only'' constraint holds end to end. The attestor
fetches the upstream over TLS~1.3 and derives the sealing key from
the RFC~5705 exporter of that session (a fresh per-seal salt prevents
key reuse across a kept-alive connection). We stress the honest
scope, elaborated in \texttt{paper/design/chacha20-circuit.md}~\S6:
this construction proves \emph{a} ChaCha20-Poly1305 record the
attestor produced from a body it fetched over an authenticated TLS
session --- not the upstream's own TLS record layer, which Go's
standard library exposes neither the keys nor the cipher-suite
control to reproduce. AES-GCM TLS sessions are therefore out of scope
by design rather than provable-but-unsupported. The in-circuit
verification of the attestor's EdDSA signature over the transcript
commitment remains a v0.3 item: the circuit runs on BLS12-381 while
the attestor signs over BN254, so v0.2 checks that signature
off-circuit.
